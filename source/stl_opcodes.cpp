...
{
	auto v_dst = brepvm::VMHelper::GetValArray(dst);
	if (src.type == brepvm::V_ARRAY)
	{
		auto v_src = brepvm::VMHelper::GetValArray(src);
		for (auto& v : *v_src) {
			vector_concat(dst, v);
		}
	}
...
void StlOpCodeImpl::VectorAdd(evm::VM* vm)
{
	...
	auto vector = brepvm::VMHelper::GetRegArray(vm, r_dst);
...
void StlOpCodeImpl::VectorGet(evm::VM* vm)
{
	...
	auto vec = brepvm::VMHelper::GetRegArray(vm, r_vec);
